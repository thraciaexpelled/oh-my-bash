...
function apply_chosen_theme {
    ...
    # check if that variable is set to 1.
    # if so, this means user has already set that theme; wrap it up
    # TODO(Sun 02 Nov 2025 02:12:00 PM, fixbug): variables are not evaluated until .bashrc is sourced
    if [[ $OMB_CONFIG_SET -eq 1 ]]; then
        error "requested theme ($theme_name) is already set"
        exit 1
    fi
    ...
    if [[ $? == 0 ]]; then
        ...
        error "cannot generate .omb-profile; theme might already be set or other error"
        exit 1
    fi
